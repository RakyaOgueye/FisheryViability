...
{
  image[0]=  r*x[0]*(1-x[0]/k)-min(x[1]+x[2],1.0)*x[0];
  image[1]=  u[0];
  image[2]=  u[1];
...
inline void jacobian(double *x, double *u , double ** jacob)
{
	...
	jacob[0][0]=r*(1-x[0]/k)-r*x[0]/k-min(x[1]+x[2],1.0);
	jacob[0][1]=x[0];
	jacob[0][2]=x[0];
...
{
  res[0]=abs(r*x[0]*(1-x[0]/k)-min(x[1]+x[2],1.0)*x[0]);
  res[1]=rakya_u1max;
  res[2]=rakya_u2max;
